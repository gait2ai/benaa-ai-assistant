# main.py (updated with model_router and database integration)
...
# Import database module from Data_Sys folder
import sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from Data_Sys.database import init_db, save_message, get_messages, delete_session, get_all_sessions
...
class RAGResponse(BaseModel):
    answer: str
    session_id: str  # Return session ID to client for tracking
...
async def process_message(user_message: UserMessage, retriever: FaissRetriever = Depends(get_retriever)):
    """Process user message using RAG with FAISS retrieval and store in database."""
    try:
        ...
        save_message(session_id, "user", user_message.message)
        ...
        return RAGResponse(answer=assistant_response, session_id=session_id)
...
async def delete_chat_history(session_id: str = Path(..., description="The session ID")):
